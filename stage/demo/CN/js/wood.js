...
function adaptiveHeight(){
  $(window).on('load resize', function(){
    var wh = $(window).height(),
        ...
        ch = $('#container div.content-all').height(); 
        window_ch = wh - hh - fh;

    var contentElement = $('#container > div.content .content-all');
    if (!$(contentElement).data('initMargin')){
        $(contentElement).data('initMargin', ~~$(contentElement).css('margin-top'))
    }
    if (ch < window_ch){
        // 只有中间内容实际显示区域小于窗口高度才作垂直居中操作
        // 43 是此元素的小间距, 至少要与顶部隔开43px
        $(contentElement).css('margin-top', (window_ch - ch) / 2 + $(contentElement).data('initMargin'));
    }
    else {
        $(contentElement).css('margin-top', $(contentElement).data('initMargin'));
    }

console.log(window_ch, ch, (window_ch - ch) / 2, $(contentElement).data('initMargin'));
